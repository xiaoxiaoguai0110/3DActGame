...
- P0 战斗稳定性：死亡状态、重力、伤害快照、场景重载单例和 Prefab 位移所有权。
- P1 动画驱动战斗：五段 Animation Event、Attack Tag、明确输入窗口和攻击配置资产。
- 作品集 P0：玩家受击硬直、有限敌人遭遇、动态目标、胜利/失败、重试和返回菜单。
- 新增本机第三方资源预检；公开仓库不重新分发第三方美术源文件。
...
- 五段连招、不同阶段伤害和武器碰撞窗口。
- 受伤、死亡、开场动画和摄像机过渡流程。
- 受击时立即关闭武器判定、中断连招并播放程序化硬直，不依赖额外付费动画资源。
...
- 默认遭遇共 8 只敌人，场上最多 5 只；目标文字实时显示击败进度。
- 最后一只敌人死亡后显示胜利界面，玩家死亡后显示失败界面，支持重试和返回菜单。
...
> 公开仓库不包含第三方美术与音频源包。完整依赖范围和本机检查方式见
> [`THIRD_PARTY_ASSETS.md`](THIRD_PARTY_ASSETS.md)。CI 只做无第三方资源的代码编译验证，
> 完整可玩版本需在拥有合法资源的本机环境构建。
